...
namespace Qk {
    ...
    struct _String
    {
        ...
        constexpr _String(const char* buf)
            : _String(Move{}, buf, strnlen(buf, 512))
        {
        }
        ...
        Unit* begin() const { return const_cast<Unit*>(_buf); }

        Unit* end() const { return const_cast<Unit*>(_buf) + _len; }

        Unit const* _buf = nullptr;
        usize       _len = 0;
    };

    static_assert(Sliceable<_String<Encoding::UTF8>>);

    using String = _String<Encoding::UTF8>;
}